  ...
  <div class="page-content">
    ...
    <div class="page-header">
      <div class="page-title">
        <h3>REDES SOCIAIS <small>LISTA</small></h3>
      </div>
    </div>
    <!-- /page header -->
    <!-- Breadcrumbs line -->
    ...
    <div class="well text-center">
                <div class="btn-toolbar text-center" role="toolbar">
                  <div class="btn-group">
                  <div class="btn-primary linkpainel" style="padding-left:10px; padding-right:10px; padding-top:7px; padding-bottom:7px;"><a href="ControleRedeSocial.php?tp=<?=$tipo_conteudo;?>">Cadastrar</a></div>
                  </div>
                </div>
              </div>
              ...
              <table class="table">
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						$sql = "SELECT * FROM rede_social WHERE (titulo_rede like '%$descpesquisa%') ORDER BY titulo_rede ASC";
				} else { 
					if ($_REQUEST['Del'] == "del") {
						$codigo = $_REQUEST['codigo'];
						$sql = "DELETE FROM rede_social WHERE id_rede = '$codigo'";
						$resultado = mysql_query($sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
						$sql = "SELECT * FROM rede_social ORDER BY id_rede DESC";
				}
				...
				$codigo 			= $linha["id_rede"];
				$titulo	        	= $linha["titulo_rede"]; 
				?>
                  <tr>
                    <td>#<?=$codigo;?></td>
                    <td><?=$titulo;?></td>
                    <td align="right"><div class="table-controls" style="float:right;"><a href="ControleRedeSocial.php?cod=<?=$codigo;?>" class="btn btn-icon btn-xs tip btn-warning" style="background:#F90" title="Editar"><i class="icon-pencil"></i></a> <a href="ConsultaRedeSocial.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" class="btn btn-danger btn-icon btn-xs tip" title="Excluir"><i class="icon-remove2"></i></a> </div></td>
                  </tr>
                  <? } ?>
                </tbody>
              </table>
