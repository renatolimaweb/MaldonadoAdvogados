...
<title>RENAN MALDONADO ADVOGADOS | PROCESSOS | CLOUDS | SISTEMA GERENCIADOR DE CONTEÚDO</title>
  ...
  <div class="page-content">
    ...
    <div class="page-header">
      <div class="page-title">
        <h3>PROCESSOS <small>LISTA</small></h3>
      </div>
    </div>
    <!-- /page header -->
    <!-- Breadcrumbs line -->
    ...
    <div class="well text-center">
                <div class="btn-toolbar text-center" role="toolbar">
                  <div class="btn-group">
                  <div class="btn-primary linkpainel" style="padding-left:10px; padding-right:10px; padding-top:7px; padding-bottom:7px;"><a href="ControleProcesso.php?tp=<?=$tipo_conteudo;?>">Cadastrar</a></div>
                  </div>
                </div>
              </div>
              ...
              <table class="table">
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						$sql = "SELECT * FROM processo, cliente WHERE processo.id_cliente = cliente.id_cliente AND (processo.titulo_processo like '%$descpesquisa%' OR cliente.nome_cliente like '%$descpesquisa%') ORDER BY processo.titulo_processo ASC, cliente.nome_cliente ASC";
				} else { 
					if ($_REQUEST['Del'] == "del") {
						$codigo = $_REQUEST['codigo'];
						$sql = "DELETE FROM processo WHERE id_processo = '$codigo'";
						$resultado = mysql_query($sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
						$sql = "SELECT * FROM processo, cliente WHERE processo.id_cliente = cliente.id_cliente ORDER BY id_processo DESC";
				}
        ...
        $nomeCliente  = $linha["nome_cliente"];
				?>
                  <tr>
                    ...
                    <td><?=$titulo;?></td>
                    <td><?=$nomeCliente;?></td>
                    <td align="right"><div class="table-controls" style="float:right;"><a href="ControleProcesso.php?cod=<?=$codigo;?>" class="btn btn-icon btn-xs tip btn-warning" style="background:#F90" title="Editar"><i class="icon-pencil"></i></a> <a href="ConsultaProcesso.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" class="btn btn-danger btn-icon btn-xs tip" title="Excluir"><i class="icon-remove2"></i></a> </div></td>
                  </tr>
                  <? } ?>
                </tbody>
              </table>
