...
<title>RENAN MALDONADO ADVOGADOS | PUBLICAÇÕES | CLOUDS | SISTEMA GERENCIADOR DE CONTEÚDO</title>
  ...
  <div class="page-content">
    ...
    <div class="page-header">
      <div class="page-title">
        <h3>PUBLICAÇÕES <small>LISTA</small></h3>
      </div>
    </div>
    <!-- /page header -->
    <!-- Breadcrumbs line -->
    ...
    <div class="well text-center">
                <div class="btn-toolbar text-center" role="toolbar">
                  <div class="btn-group">
                  <div class="btn-primary linkpainel" style="padding-left:10px; padding-right:10px; padding-top:7px; padding-bottom:7px;"><a href="ControlePublicacao.php?tp=<?=$tipo_conteudo;?>">Cadastrar</a></div>
                  </div>
                </div>
              </div>
              ...
              <table class="table">
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						$sql = "SELECT * FROM publicacao WHERE (titulo_publicacao like '%$descpesquisa%') ORDER BY titulo_publicacao ASC";
				} else { 
					if ($_REQUEST['Del'] == "del") {
						$codigo = $_REQUEST['codigo'];
						$sql = "DELETE FROM publicacao WHERE id_publicacao = '$codigo'";
				...
				$codigo 			= $linha["id_publicacao"];
				$titulo	        	= $linha["titulo_publicacao"];
				...
				?>
                  <tr>
                    ...
                    <td><?=$data;?> </td>
                    <td><?=$titulo;?></td>
                    <td align="right"><div class="table-controls" style="float:right;"><a href="ControlePublicacao.php?cod=<?=$codigo;?>" class="btn btn-icon btn-xs tip btn-warning" style="background:#F90" title="Editar"><i class="icon-pencil"></i></a> <a href="ConsultaPublicacao.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" class="btn btn-danger btn-icon btn-xs tip" title="Excluir"><i class="icon-remove2"></i></a> </div></td>
                  </tr>
                  <? } ?>
                </tbody>
              </table>
