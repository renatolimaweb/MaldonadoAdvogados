  ...
  <div class="page-content">
    ...
    <div class="page-header">
      <div class="page-title">
        <h3>ATUALIZAÇÕES DOS PROCESSOS <small>LISTA</small></h3>
      </div>
    </div>
    <!-- /page header -->
    <!-- Breadcrumbs line -->
    <div class="breadcrumb-line">
      <ul class="breadcrumb">
        ...
        <li class="active">ATUALIZAÇÕES DOS PROCESSOS</li>
      </ul>
    ...
    <div class="well text-center">
                <div class="btn-toolbar text-center" role="toolbar">
                  <div class="btn-group">
                  <div class="btn-primary linkpainel" style="padding-left:10px; padding-right:10px; padding-top:7px; padding-bottom:7px;"><a href="ControleDialogoProcesso.php?tp=<?=$tipo_conteudo;?>">Cadastrar</a></div>
                  </div>
                </div>
              </div>
              ...
              <table class="table">
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						$sql = "SELECT * FROM dialogo_processo, processo WHERE dialogo_processo.id_processo = processo.id_processo AND (processo.titulo_processo like '%$descpesquisa%') ORDER BY processo.titulo_processo ASC";
				} else { 
					if ($_REQUEST['Del'] == "del") {
						$codigo = $_REQUEST['codigo'];
						$sql = "DELETE FROM dialogo_processo WHERE id_dialogo_processo = '$codigo'";
						$resultado = mysql_query($sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
						$sql = "SELECT * FROM dialogo_processo, processo WHERE dialogo_processo.id_processo = processo.id_processo ORDER BY dialogo_processo.id_dialogo_processo DESC";
				}
				...
				$codigo 			= $linha["id_dialogo_processo"];
				$titulo	        	= $linha["titulo_processo"];
                $data   			= $linha["data_dialogo_processo"];
				...
				?>
                  <tr>
                    ...
                    <td><?=$data;?></td>
                    <td><?=$titulo;?></td>
                    <td align="right"><div class="table-controls" style="float:right;"><a href="ControleDialogoProcesso.php?cod=<?=$codigo;?>" class="btn btn-icon btn-xs tip btn-warning" style="background:#F90" title="Editar"><i class="icon-pencil"></i></a> <a href="ConsultaDialogoProcesso.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" class="btn btn-danger btn-icon btn-xs tip" title="Excluir"><i class="icon-remove2"></i></a> </div></td>
                  </tr>
                  <? } ?>
                </tbody>
              </table>
