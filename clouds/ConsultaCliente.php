  ...
  <div class="page-content">
    ...
    <div class="well text-center">
                <div class="btn-toolbar text-center" role="toolbar">
                  <div class="btn-group">
                  <div class="btn-primary linkpainel" style="padding-left:10px; padding-right:10px; padding-top:7px; padding-bottom:7px;"><a href="ControleCliente.php?tp=<?=$tipo_conteudo;?>">Cadastrar</a></div>
                  </div>
                </div>
              </div>
              ...
              <table class="table">
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						$sql = "SELECT * FROM cliente WHERE (nome OR email_cliente like '%$descpesquisa%') ORDER BY nome_cliente OR email_cliente ASC";
				} else { 
					if ($_REQUEST['Del'] == "del") {
				...
				$titulo	        	= $linha["nome_cliente"];
				?>
                  <tr>
                    <td>#<?=$codigo;?></td>
                    <td><?=$titulo;?></td>
                    <td align="right"><div class="table-controls" style="float:right;"><a href="ControleCliente.php?cod=<?=$codigo;?>" class="btn btn-icon btn-xs tip btn-warning" style="background:#F90" title="Editar"><i class="icon-pencil"></i></a> <a href="ConsultaCliente.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" class="btn btn-danger btn-icon btn-xs tip" title="Excluir"><i class="icon-remove2"></i></a> </div></td>
                  </tr>
                  <? } ?>
                </tbody>
              </table>
